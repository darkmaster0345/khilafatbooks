...
CREATE OR REPLACE FUNCTION public.create_verified_order(
  ...
  p_customer_email text DEFAULT NULL, p_delivery_address text DEFAULT NULL,
  p_delivery_city text DEFAULT NULL, p_payment_screenshot_url text DEFAULT NULL,
  p_transaction_id text DEFAULT NULL, p_zakat_enabled boolean DEFAULT false,
  p_discount_code text DEFAULT NULL, p_referral_discount integer DEFAULT 0,
  p_recovery_discount integer DEFAULT 0, p_is_gift boolean DEFAULT false,
  p_gift_recipient_name text DEFAULT NULL, p_gift_message text DEFAULT NULL,
  p_gift_wrap boolean DEFAULT false, p_referral_code_id uuid DEFAULT NULL,
...
DECLARE
  ...
  v_has_custom_delivery boolean := false;
  v_has_default_delivery boolean := false;
  v_custom_delivery_total integer := 0;
BEGIN
  v_user_id := auth.uid();
  ...
  LOOP
    SELECT id, name, price, type, in_stock, delivery_price INTO v_product
    ...
    -- Track delivery pricing per product
    IF v_product.type = 'physical' THEN
      v_has_physical := true;
      IF v_product.delivery_price IS NOT NULL THEN
        v_has_custom_delivery := true;
        v_custom_delivery_total := v_custom_delivery_total + (v_product.delivery_price * COALESCE((v_item->>'quantity')::int, 1));
      ELSE
        v_has_default_delivery := true;
      END IF;
    END IF;
  ...
  -- Calculate shipping: use per-product prices when all physical items have custom delivery
  IF v_has_physical THEN
    IF v_has_custom_delivery AND NOT v_has_default_delivery THEN
      -- All physical items have custom delivery prices
      v_shipping := v_custom_delivery_total;
    ELSIF v_has_custom_delivery AND v_has_default_delivery THEN
      -- Mix: custom delivery items + default items
      v_shipping := v_custom_delivery_total;
      IF v_subtotal < 5000 THEN
        v_shipping := v_shipping + 500; -- Add flat rate for default items
      END IF;
    ELSE
      -- All default: flat rate
      IF v_subtotal < 5000 THEN
        v_shipping := 500;
      END IF;
    END IF;
