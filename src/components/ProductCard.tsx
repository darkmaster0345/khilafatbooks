...
const ProductCard = ({ product, index = 0 }: { product: LegacyProduct; index?: number }) => {
  ...
  const handleGiftClick = (e: React.MouseEvent) => {
    ...
    toast({
      ...
      description: 'You can be notified when it becomes available for gifting.',
    });
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <Link to={`/books/${product.slug}`}>
            <h3 className="mt-1.5 font-display text-[15px] font-semibold text-foreground line-clamp-2 hover:text-primary transition-colors leading-snug h-[2.8em]">
              {product.name}
            </h3>
          </Link>
          {product.nameAr && (
          ...
          <div className="mt-auto pt-3 border-t border-border/50 mt-3">
            ...
            <Button
              size="sm"
              onClick={handleNotifyMe}
              className="h-9 gap-1.5 text-xs rounded-xl shadow-sm hover:shadow-md transition-all md:hidden gold-gradient border-0 text-foreground font-bold"
            >
              <BellRing className="h-3.5 w-3.5" />
              Notify
            </Button>
